...
(() => {
  ...
  const allText = (sel, root = document) =>
  ...
  const num = (s) => {
    ...
    if (lastDot !== -1 && lastComma !== -1) {
      ...
      if (parts.length > 2 || tail.length === 3) t = parts.join('');
      else t = parts.join('.');
    }
    const n = Number(t);
  ...
  function description() {
    const full = text('.comment .adCommentsLanguage') || text('.comment div') || text('.comment');
    return full;
  }

  function features() {
    const groups = [...document.querySelectorAll('.details-property-feature-one, .details-property-feature-two, .info-features')];
  ...
  function agencyInfo() {
    ...
    if (!link) return { name: null, profile_url: null };
  ...
  function roomDetail(feats) {
    const find = (re) => feats.find((f) => re.test(f)) || null;
  ...
  function extract() {
    ...
    const photoCount =
      document.querySelectorAll('.image-gallery img, .detail-multimedia img').length ||
      (/listing without photos|anuncio sin fotos/i.test(body) ? 0 : 0);
    const phoneAvailable = !/they have not provided a phone number|no ha facilitado (un )?tel[eé]fono/i.test(
      body
    );
    const ad = u.ad || {};
    ...
    return {
      ...
      portal: 'idealista',
      listing_id: ad.id || (location.pathname.match(/\d{5,}/) || [null])[0],
      ...
      title: text('.main-info__title-main') || text('h1') || document.title,
      location_line: text('.main-info__title-minor') || null,
      city: (ad.address && ad.address.municipality) || null,
      district: (ad.address && (ad.address.district || ad.address.locationName)) || null,
      ...
      advertiser_type:
        (ad.owner && ad.owner.type) ||
        (agency.profile_url || document.querySelector('.professional-name') ? 'professional' : 'private'),
      advertiser_name: agency.name || text('.professional-name .name') || text('.advertiser-name-container'),
  ...
  window.__pisocheckExtract = { extract, isListingPage };
})();
